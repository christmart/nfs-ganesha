...
static fattr_xdr_result encode_fsid(XDR *xdr, struct xdr_attrs_args *args)
{
	...
	if (args->data != NULL &&
		...
		LogDebug(COMPONENT_NFS_V4, "args->data != NULL and EXPORT_OPTION_FSID_SET" );
		fsid.major = op_ctx->ctx_export->filesystem_id.major;
		fsid.minor = op_ctx->ctx_export->filesystem_id.minor;
	} else {
		uint64_t exportid = 0;
		fsal_status_t st;
		LogDebug(COMPONENT_NFS_V4, "module_path = %s, module_name = %s",
			 args->data->current_obj->fsal->path,
			 args->data->current_obj->fsal->name);
		st = args->data->current_obj->
			obj_ops.fs_loc_exportid( args->data->current_obj,
						 &exportid);
		LogDebug(COMPONENT_NFS_V4, "st.minor = %d, st.major = %d", st.minor, st.major);
		if ( st.minor == 0 && st.major == 0 && exportid > 0) {
			LogDebug(COMPONENT_NFS_V4, "fs_loc_exportid %"PRIu64, exportid);
			fsid.major = exportid;
			fsid.minor = exportid;
		} else {
	...
	LogDebug(COMPONENT_NFS_V4, "fsid.major = %"PRIu64", fsid.minor = %"PRIu64, fsid.major, fsid.minor );
...
void nfs4_pathname4_alloc(pathname4 *pathname4, char *path)
{
	...
	if (path == NULL) {
		...
		pathname4->pathname4_val->utf8string_val = gsh_calloc(MAXPATHLEN, sizeof(char));
		...
		LogEvent(COMPONENT_NFSPROTO, "%s has %d tokens", path, i);
		...
		while ( (token = strsep(&path_work, "/")) != NULL ) {
			if ( strlen(token) > 0) {
				LogEvent(COMPONENT_NFSPROTO, "token %d is %s", i, token);
				pathname4->pathname4_val[i].utf8string_val = gsh_strdup(token);
				pathname4->pathname4_val[i].utf8string_len = strlen(token);
				i++;
			}
		}
...
void nfs4_pathname4_free(pathname4 *pathname4)
{
	...
	if (pathname4 != NULL) {
		...
		LogDebug(COMPONENT_NFSPROTO, "number of pathname components to free: %d", i);

		if (pathname4->pathname4_val != NULL) {
			while (i-- > 0) {
				if (pathname4->pathname4_val[i].utf8string_val != NULL ) {
					LogDebug(COMPONENT_NFSPROTO, "freeing component %d: %s", i+1,
					    ...
					    struct xdr_attrs_args *args)
{
	...
	LogEvent(COMPONENT_NFSPROTO, "encode_fs_locations");
	...
	*/
	...
	if (FSAL_IS_ERROR(st)) {
		...
		strcpy(fs_locs.fs_root.pathname4_val->utf8string_val, "not_supported");
		strcpy(fs_loc.rootpath.pathname4_val->utf8string_val, "not_supported");
		...
		LogEvent(COMPONENT_NFSPROTO, "encode_fs_locations obj_ops.fs_locations failed %s, %s, %s",
	...
	if (!xdr_fs_locations4(xdr, &fs_locs))
	  {
	    LogEvent(COMPONENT_NFSPROTO, "encode_fs_locations xdr_fs_locations failed %s, %s, %s",
		     fs_locs.fs_root.pathname4_val->utf8string_val,
		     fs_loc.rootpath.pathname4_val->utf8string_val,
		     server);
