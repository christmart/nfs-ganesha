			    ...
			    struct attrlist *attrs_out)
{
	...
	LogDebug(COMPONENT_FSAL, "fh=%p", fh);
	...
	*handle = NULL;		/* poison it first */
	...
	fd = (hdl->obj_handle.type == DIRECTORY) ?
	    vfs_fsal_open(hdl, O_DIRECTORY, &fsal_error) :
	    vfs_fsal_open(hdl, O_RDWR, &fsal_error);
	if (fd < 0) {
		close(fd);
		return fsalstat(fsal_error, -fd);
	}

	if (fd > 0 && hdl->obj_handle.type == DIRECTORY ) {
		...
		if (hdl->u.directory.path != NULL) {
			LogEvent(COMPONENT_FSAL, "freeing old directory.path: %s",
		...
		if (status.major == ERR_FSAL_NO_ERROR) {
			...
			if (hdl->u.directory.fs_location != NULL) {
				LogEvent(COMPONENT_FSAL, "freeing old directory.fs_location: %s",
					 hdl->u.directory.fs_location);
				gsh_free(hdl->u.directory.fs_location);
			}
			hdl->u.directory.fs_location = gsh_strdup(xattr_content);
			hdl->u.directory.exportid = DJBHash(xattr_content, attrsize);

			//struct gsh_export *export = export_init(NULL, NULL);
			//export->fullpath = gsh_strdup(hdl->u.directory.path);
			//export->pseudopath = gsh_strdup(hdl->u.directory.path);
			//add_export_commit(node, NULL, export, err_type);
			//fsal_init
			//fsal_cfg_commit
		}
					...
					struct fs_locations4 *fs_locs)
{
	...
	LogEvent(COMPONENT_FSAL,
		 ...
		 vfs_fs->fs->path, vfs_fs->root_fd, (int)vfs_fs->fs->fsid.major, (int)vfs_fs->fs->fsid.minor);
	...
	LogDebug(COMPONENT_FSAL,
		 ...
		 myself->u.directory.fs_location);
	if (myself->u.directory.fs_location != NULL) {
		char *server, *rootpath;
		char *path_sav, *path_work;

		//strcpy(&path, myself->u.directory.fs_location);
		...
		rootpath = myself->u.directory.path;
		nfs4_pathname4_free(&fs_locs->fs_root);
		nfs4_pathname4_alloc(&fs_locs->fs_root, rootpath);
		...
		nfs4_pathname4_alloc(&fs_locs->locations.locations_val->rootpath, path_work);
...
/* vfs_fs_loc_exportid
 */
static fsal_status_t vfs_fs_loc_exportid(struct fsal_obj_handle *obj_hdl,
					 uint64_t *exportid)
{
	struct vfs_fsal_obj_handle *myself;
	...
	LogDebug(COMPONENT_FSAL,
		 "exportid = %"PRIu64,
		 myself->u.directory.exportid);
	*exportid = myself->u.directory.exportid;
...
void vfs_handle_ops_init(struct fsal_obj_ops *ops)
{
	...
	ops->fs_loc_exportid = vfs_fs_loc_exportid;
	ops->close = vfs_close;
